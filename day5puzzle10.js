...
function mySeat(seatId) {
  let mySeatId = null;
  let sortedIds = seatId.sort(function(a,b){return a - b});
  for (p=2; p < sortedIds.length; p++) {
    if (sortedIds[p] !== sortedIds[p-1]+1) {
      console.log(`Your ID is ${sortedIds[p]-1}`)
    }
  }
  return mySeatId;
}

function findMySeat(ticket) {
  let seatId = seatIdFinder(ticket);
  let mySeatId = mySeat(seatId);
}
