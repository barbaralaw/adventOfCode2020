...
function groupAgree(allGrpsAns) {
  let partyAnswers = [];
  let ttlAgree = 0;
  for (party = 0; party < allGrpsAns.length; party += 1) {
    partyAnswers = allGrpsAns[party];
    /*
    if (partyAnswers.length === -1 ) {
      ttlAgree += partyAnswers[0].length ;
      console.log(ttlAgree);
    }  else {
    */
      firstPerson = partyAnswers[0]
      for (question = 0; question < firstPerson.length; question += 1) {
        let valid = true;
        questionToChk = firstPerson.charAt(question)
        for (person = 1; person < partyAnswers.length; person += 1) {
          personAns = partyAnswers[person];
          if (personAns.includes(questionToChk) === false) {
            valid = false;
            break;
          }
        }
